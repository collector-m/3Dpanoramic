...
{   
  ...
  //int m_height = (src1->height)*2;
  //int m_width = (src1->width)*2;

  //CvMat* mapx_persp = cvCreateMat(m_height, m_width, CV_32FC1);
  //CvMat* mapy_persp = cvCreateMat(m_height, m_width, CV_32FC1);
  ...
  //IplImage *dst_persp   = cvCreateImage( cvSize(m_width, m_height), 8, 3 );   // undistorted perspective and panoramic image
  ...
  cvShowImage("1", mapx_persp);
  cvShowImage("2", mapy_persp);
  cvWaitKey(0);
